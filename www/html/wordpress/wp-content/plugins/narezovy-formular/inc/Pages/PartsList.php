...
class PartsList extends RenderEditor {
    ...
    private function render_table_content(){
        ...
        foreach ($this->parts as $row) {
            ...
            echo '<td style="width: 9%">' .$row->sirka_dilu .'</td>';
            echo '<td style="width: 9%">' .($row->hrana_dolni != 0 ? $this->get_hrana_name_by_id($row->hrana_dolni, true) : '') .'</td>';
            echo '<td style="width: 9%">' .($row->hrana_horni != 0 ? $this->get_hrana_name_by_id($row->hrana_horni, true) : '') .'</td>';
            echo '<td style="width: 9%">' .($row->hrana_prava != 0 ? $this->get_hrana_name_by_id($row->hrana_prava, true) : '') .'</td>';
            echo '<td style="width: 9%">' .($row->hrana_leva != 0 ? $this->get_hrana_name_by_id($row->hrana_leva, true) : '') .'</td>';
            ...
            <div style="overflow-x: auto;">
                <table class="shop_table cart wishlist_table wishlist_view traditional responsive">
                    <thead style="display: block;" class="th-middle">
                        ...
                        <th style="width: 9%">Šířka dílu</th>
                        <th style="width: 9%">Hrana přední</th>
                        <th style="width: 9%">Hrana zadní</th>
                        <th style="width: 9%">Hrana pravá</th>
                        <th style="width: 9%">Hrana levá</th>
