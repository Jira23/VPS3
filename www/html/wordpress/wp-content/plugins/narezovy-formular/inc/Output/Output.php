...
//                $re = new \Inc\Pages\ClassicEditor\RenderEditor();                
                ...
                foreach ($dily_data as $row) {                                       // vypis dat
                    ...
                    .'<td>' .($row->hrana_dolni != 0 ? $rp->get_hrana_name_by_id($row, true, true) : '')  .'</td>'
                    .'<td>' .($row->hrana_horni != 0 ? $rp->get_hrana_name_by_id($row, true, true) : '') .'</td>'                                    
                    .'<td>' .($row->hrana_prava != 0 ? $rp->get_hrana_name_by_id($row, true, true) : '') .'</td>'
                    .'<td>' .($row->hrana_leva != 0 ? $rp->get_hrana_name_by_id($row, true, true) : '') .'</td>'
                    .'<td>' .$row->tupl .'</td>'
